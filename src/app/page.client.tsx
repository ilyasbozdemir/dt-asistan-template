...
import { Menu, X, FileText, Settings, Download, Edit, LayoutDashboard, FileArchive, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className="flex h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-50 overflow-hidden font-sans">
      ...
      `}>
        {/* Sidebar Header */}
        <div className={`h-16 flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-between px-6'} bg-slate-950 border-b border-slate-800 flex-shrink-0`}>
          {!sidebarCollapsed && (
            <div className="flex items-center gap-3 text-white">
              <LayoutDashboard className="w-6 h-6 text-blue-500 flex-shrink-0" />
              <h1 className="text-xl font-bold tracking-tight whitespace-nowrap overflow-hidden">Şablonlar</h1>
            </div>
          )}
          {sidebarCollapsed && (
             <LayoutDashboard className="w-6 h-6 text-blue-500 flex-shrink-0" />
        ...
        <div className="flex-1 overflow-y-auto p-3 scrollbar-thin scrollbar-thumb-slate-700">
          <div className="flex flex-col gap-6 mt-2">
            ...
            <div className="space-y-2">
              ...
              {sidebarCollapsed ? (
                 <div className="flex justify-center text-slate-500 cursor-help" title={activeCategory}>
                ...
                <div className="relative">
                  ...
                  >
                    {Object.keys(groupedTemplates).sort().map((cat) => {
                      ...
                      else if (cat.includes("2-")) niceName = "2. Piyasa Fiyat";
                  ...
                  groupedTemplates[activeCategory].map((t) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center ${sidebarCollapsed ? 'justify-center p-3' : 'gap-3 px-3 py-2.5'} rounded-lg text-sm font-medium transition-all ${
                          isActive
                            ? "bg-blue-600 text-white shadow-md shadow-blue-900/20"
                            : "text-slate-400 hover:bg-slate-800 hover:text-slate-200"
                        }`}
                      >
                        <FileText className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-blue-200' : 'text-slate-500'}`} />
                        {!sidebarCollapsed && (
                          <span className="truncate whitespace-nowrap text-left w-full">{t.name.replace(/([A-Z])/g, " $1").trim()}</span>
                        )}
                      </button>
                    );
                  })}
                    ...
                    <div key={key} className="flex flex-col gap-1.5">
                      ...
                      {isBoolean ? (
                        ...
                        <textarea
                          ...
                          onBlur={(e) => handleJsonChange(key, e.target.value)}
